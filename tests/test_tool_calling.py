"""
test_tool_calling.py : couvre le module tool_schemas et l'intégration
ClaudeClient._emit_tool_events_from_final (Phase A.7.2 v15, MT).

Tests unitaires uniquement, pas d'appels réseau. Valide :
- Les 3 schémas Anthropic / Gemini / OpenAI-compat ont la bonne forme
- engine_supports_native_tools matche les engines listés
- tool_call_to_payload retourne le bon format pour chaque tool
- tune_prompt_for_engine préfixe correctement par moteur
- _emit_tool_events_from_final convertit un mock de final message en
  ParserEvent typés
...
class TestToolSchemas(unittest.TestCase):
    ...
    def test_gemini_function_declarations_format(self):
        ...
        for d in decls:
            ...
            # Gemini utilise "parameters" (au lieu d'"input_schema" Anthropic)
...
# ============================================================ Mock _emit_tool_events_from_final

class TestEmitToolEvents(unittest.TestCase):

    def setUp(self):
        # Importe ClaudeClient en mode minimal (pas d'API call)
    ...
    def _make_final(self, blocks):
        """Construit un mock de final message avec une liste de blocks."""
        mock = MagicMock()
        mock.content = blocks
        return mock
