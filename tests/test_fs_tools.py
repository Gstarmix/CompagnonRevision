"""test_fs_tools.py : couvre le module fs_tools (Phase A.12).

Valide les schémas (3 formats moteur) et l'exécuteur réel d'outils FS :
Read / Grep / Glob, scoping sécurité, ingestion PDF/image, erreurs douces.

Pas d'appel réseau : tout est sur un dossier temporaire local.

Lance :
    python -m unittest tests.test_fs_tools
"""
...
from fs_tools import (  # noqa: E402
    FS_TOOL_NAMES,
    TOOL_MARKER_CLOSE,
    TOOL_MARKER_OPEN,
    FsToolResult,
...
class TestSchemas(unittest.TestCase):
    ...
    def test_gemini_types_are_uppercase(self):
        """Gemini exige l'enum Type en MAJUSCULES (OBJECT/STRING/...)."""
        decls = gemini_fs_declarations()
        for d in decls:
            params = d["parameters"]
...
class TestExecute(unittest.TestCase):
    ...
    def test_read_text_file_numbered(self):
        ...
        self.assertIn("1\t", res.text)  # numérotation de ligne
    ...
    def test_read_pdf_returns_document(self):
        # En-tête PDF minimal : il suffit, on ne parse pas, on ingère le binaire.
    ...
    def test_execute_never_raises(self):
        # Args farfelus : aucun appel ne doit propager d'exception.
...
class TestToolCallMarker(unittest.TestCase):
    ...
    def test_marker_single_line(self):
        # Le marqueur doit tenir sur une seule ligne (injecté dans le flux).
        self.assertNotIn("\n", tool_call_marker("Read", "a/b/c.tex", True))

    def test_marker_delimiters_distinct_from_parser_close_tag(self):
        # Les délimiteurs ne doivent pas être la balise <<<END>>> du parser.
